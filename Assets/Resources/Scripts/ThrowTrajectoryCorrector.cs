...
public class ThrowTrajectoryCorrector : MonoBehaviour
{
    ...
    void AdvanceAlongPath(float deltaTime)
    {
        ...
        if (traveledDistance >= totalPathLength)
        {
            ...
            float overflow = traveledDistance - totalPathLength;
            transform.position = FlattenToFlightHeight(
                pathPoints[pathPoints.Length - 1] + throwDirection * overflow
            );
            return;
        }

        Vector3 previousPosition = transform.position;
        transform.position = FlattenToFlightHeight(SamplePositionAtDistance(traveledDistance));

        if (alignRotationToVelocity)
        {
            Vector3 velocityDirection = transform.position - previousPosition;

            if (velocityDirection.sqrMagnitude > 0.0001f)
            {
                transform.rotation = Quaternion.LookRotation(velocityDirection.normalized, Vector3.up);
            }
        }
    }

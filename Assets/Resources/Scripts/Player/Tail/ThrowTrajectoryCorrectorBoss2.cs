...
/// 補正軌道の終点に着いても停止せず、投げた方向へそのまま飛び続ける。
/// </summary>
[DisallowMultipleComponent]
[RequireComponent(typeof(Rigidbody))]
public class ThrowTrajectoryCorrectorBoss2 : MonoBehaviour
{
    [Header("ターゲット")]
    ...
    void ResolveThrowTarget()
    {
        ...
        {
            GameObject foundByTag = GameObject.FindGameObjectWithTag(targetTag);

            if (foundByTag != null)
            {
                throwTarget = foundByTag;
                return;
            }
        }
    ...
    void BuildArcLengthTable()
    {
        ...
        {
            pathPoints = new Vector3[2];
            pathPoints[0] = FlattenToFlightHeight(transform.position);
            pathPoints[1] = pathPoints[0] + throwDirection * maxFlightDistance;

            cumulativeLengths = new float[2];
            cumulativeLengths[0] = 0f;
            cumulativeLengths[1] = Vector3.Distance(pathPoints[0], pathPoints[1]);
            totalPathLength = cumulativeLengths[1];
        }
    }
    ...
    void AdvanceAlongPath(float deltaTime)
    {
        if (totalPathLength <= 0.0001f)
        {
            ApplyVelocityAlongPath(throwDirection);
            return;
        }
        ...
        {
            tangent = throwDirection;
        }
        else
        {
            tangent = GetPathTangent(traveledDistance);
        }
    ...
    void ApplyVelocityAlongPath(Vector3 direction)
    {
        ...
        if (direction.sqrMagnitude <= 0.0001f)
        {
            direction = FlattenDirection(throwDirection);
        }

        if (direction.sqrMagnitude <= 0.0001f)
        {
            direction = FlattenDirection(transform.forward);
        }
        ...
        if (alignRotationToVelocity && direction.sqrMagnitude > 0.0001f)
        {
    ...
    Vector3 GetPathTangent(float distance)
    {
        ...
        Vector3 tangent = FlattenDirection(ahead - current);
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if (state != FlightState.Idle && hasLockedGoal)
        {
